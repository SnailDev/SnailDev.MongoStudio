...
(function ($) {
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function () {
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' value='1' style='width:20px' maxlength='7' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        loadServerData: function (param, clause) {
            ...
            var ajaxOptions = {
                ...
                success: function (data) {
                    // 此处做了修改
                    data[p.root] = $.parseJSON(data[p.root]);
                    g.set("columns", data[p.column]);

                    g.trigger('success', [data, g]);
                    if (!data || !data[p.root] || !data[p.root].length) {
                        // 此处做了修改
                        g.changeGridHeader(0);
                    ...
                    if (g.data[p.record] != null) {
                        ...
                        // 此处做了修改
                        g.changeGridHeader(g.cacheData.records);
                    }
        ...
        // 新增改变表头的方法
        changeGridHeader: function (length) {
            var str = length > 1 ? "Documents" : "Document";
            $(".l-panel-header-text").text(length + " " + str);
        },
...
(function ($) {
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        selectNode: function (selectNodeParm, flag) {
            var g = this, p = this.options;
            var clause = null;
            if (typeof (selectNodeParm) == "function") {
                ...
                g.trigger('select', [{ data: treenodedata, target: treeitembody.parent().get(0), flag: flag }]);
                return;
            }
            else {
                ...
                if (!$(obj).hasClass("l-checkbox") && !clickOnTreeItemBtn) {
                    if (!treeitem.hasClass("l-unselectable")) {
                        if ($(">div:first", treeitem).hasClass("l-selected") && p.needCancel) {
                            ...
                            g.trigger('select', [{ data: treenodedata, target: treeitem[0], flag: true }])
                        }
                    }
                }
                //chekcbox even
